...
class Ner:
    ...
    def load_model(self, model_dir: str, model_config: str = "model_config.json"):
        ...
        model.load_state_dict(torch.load(
            output_model_file, map_location='cpu'))
    ...
    def predict(self, text: str):
        input_ids, input_mask, segment_ids, valid_positions = self.preprocess(
            text)
        ...
        logits_confidence = [values[label].item()
                             for values, label in zip(logits[0], logits_label[0])]

        logits_label = [logits_label[0][index]
                        for index, i in enumerate(input_mask[0]) if i.item() == 1]
        logits_label.pop(0)
        logits_label.pop()
        ...
        output = {word: {"tag": label, "confidence": confidence}
